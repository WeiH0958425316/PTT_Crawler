...
ptt_crawler_download <- function(search_board, 
      ...
      if (missing(proxy)) {
            ...
            last_page <- 
                  html_session(sprintf(url,  ""), use_cookies, proxy)
      }
      last_page <- 
            last_page %>% 
            read_html()  %>% 
            ...
            error_list[[length(error_list) + 1]] <- 
                  try({
                        if (missing(proxy)) {
                              ...
                              page_html <- 
                                    html_session(sprintf(url, page_i), use_cookies, proxy) %>% 
                                    read_html()
                        }
                        ...
                        article_title <- 
                              ...
                              mutate(ARTICLE_ID = ifelse(ARTICLE_ID == TITLE, NA, 
                                     ...
                                     URL = ifelse(URL == TITLE, NA, paste0("https://www.ptt.cc", URL))
                                     )
            ...
            error_list[[length(error_list) + 1]] <- 
                  try({
                        if (missing(proxy)) {
                              ...
                              page_html <- 
                                    html_session(total_article_title$URL[total_article_title_i], use_cookies, proxy)
                        }
